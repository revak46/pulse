...
IDEKA_CARDS = {
    ...
    "2026-03-12": ("for a wednesday kind of energy", "Some days just have better energy than others. Today feels like one of those days.", "for you today"),
    ...
    "2026-03-18": ("for a tuesday plot twist", "Plot twist: The person who keeps showing up, paying attention, and remembering everything? That is the main character energy right there.", "for you today"),
    "2026-03-19": ("for a wednesday worth noting", "To be truly known by someone — not performed for, not managed, just known — that is one of the rarest gifts in a noisy world.", "for you today"),
}

YEMI_CARDS = {
    "2026-03-10": ("concern but moderate", "Concern that knows its own name is already halfway to peace. You are more aware than you think.", "for you today"),
    "2026-03-11": ("for the builders", "Every great builder has a season where nothing looks finished. That is not failure. That is foundations.", "for you today"),
    ...
    "2026-03-13": ("for the intentional", "Clarity does not always arrive loudly. Sometimes it settles in like morning light — slowly, then all at once.", "for you today"),
    ...
    "2026-03-18": ("for the in between", "Not every day has a colour yet. Some days are still deciding. That is fine — so are you.", "for you today"),
    "2026-03-19": ("keep moving", "You do not have to have it all figured out. You just have to keep moving with intention.", "for you today"),
}
...
<style>
  *, *::before, *::after {{ margin: 0; padding: 0; box-sizing: border-box; }}
  ...
  body {{ font-family: 'Cormorant Garamond', serif; background: linear-gradient(160deg, #2c1a00 0%, #1a0f00 100%); min-height: 100vh; min-height: 100dvh; display: flex; flex-direction: column; align-items: center; justify-content: space-between; padding: 52px 36px 48px; position: relative; overflow: hidden; text-align: center; }}
...
<style>
  *, *::before, *::after {{ margin: 0; padding: 0; box-sizing: border-box; }}
  ...
  .yemi-section {{ min-height: 100vh; min-height: 100dvh; background: linear-gradient(160deg, #0d0d1a 0%, #0a0a0a 100%); display: flex; flex-direction: column; align-items: center; justify-content: space-between; padding: 52px 36px 48px; position: relative; overflow: hidden; text-align: center; }}
  ...
  .scroll-hint {{ position: absolute; bottom: 80px; left: 50%; transform: translateX(-50%); font-family: 'Montserrat', sans-serif; font-size: 8px; letter-spacing: 3px; text-transform: uppercase; color: rgba(255,255,255,0.2); animation: bounce 2s ease-in-out infinite; }}
  .section-divider {{ background: #0a0a0a; padding: 24px 36px; display: flex; align-items: center; gap: 16px; }}
  .section-divider::before, .section-divider::after {{ content: ''; flex: 1; height: 1px; background: linear-gradient(90deg, transparent, rgba(255,255,255,0.1), transparent); }}
  .divider-label {{ font-family: 'Montserrat', sans-serif; font-size: 8px; letter-spacing: 4px; color: rgba(255,255,255,0.2); text-transform: uppercase; white-space: nowrap; }}
  .send-section {{ background: #0a0a0a; padding: 0 36px 24px; display: flex; justify-content: center; }}
  .send-btn {{ font-family: 'Montserrat', sans-serif; font-size: 9px; font-weight: 400; letter-spacing: 4px; text-transform: uppercase; color: #c8922a; background: rgba(200,146,42,0.08); border: 1px solid rgba(200,146,42,0.3); border-radius: 2px; padding: 12px 28px; cursor: pointer; text-decoration: none; display: inline-block; }}
  .ideka-section {{ min-height: 100vh; min-height: 100dvh; background: linear-gradient(160deg, #2c1a00 0%, #1a0f00 100%); display: flex; flex-direction: column; align-items: center; justify-content: space-between; padding: 52px 36px 48px; position: relative; overflow: hidden; text-align: center; }}
...
<div class="yemi-section">
  ...
  <div class="y-bottom">
    <div class="y-dot"></div>
    <div class="y-date">{pd}</div>
  </div>
  <div class="scroll-hint">↓ her card</div>
</div>
<div class="section-divider"><span class="divider-label">her card today</span></div>
...
def main():
    ...
    open(os.path.join(pulse_dir, "index.html"), "w").write(build_ideka_html(i_label, i_quote, i_attribution, today))
    open(os.path.join(pulse_dir, "me.html"), "w").write(build_yemi_html(y_label, y_quote, y_attribution, i_label, i_quote, i_attribution, today))
    ...
    print("Your card:  https://pulse-ruddy-three.vercel.app/me.html")
    print("Her card:   https://pulse-ruddy-three.vercel.app")
