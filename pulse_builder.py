...
<style>
  *, *::before, *::after {{ margin: 0; padding: 0; box-sizing: border-box; }}
  ...
  /* ── TEXT GLITTER — the words themselves shimmer ── */
  .quote {{
    background: linear-gradient(90deg,
      #c8922a  0%,
      #f5e6c8 10%,
...
(function(){{
  ...
  /* ── drifting particles — gold dust rising ── */
  var driftCols=[
    'rgba(255,210,50,0.95)',
    'rgba(232,184,75,0.92)',
    'rgba(255,235,120,0.88)',
    'rgba(255,200,40,0.85)',
    'rgba(248,220,90,0.80)'
  ];
  for(var i=0;i<26;i++){{
    ...
    p.style.cssText=[
      ...
      'background:'+col,
      'border-radius:50%',
      'box-shadow:0 0 '+glow+'px '+glow+'px '+glowCol,
      'animation:particleDrift '+(Math.random()*8+6).toFixed(1)+'s ease-in-out '+(Math.random()*18).toFixed(1)+'s infinite',
      'pointer-events:none'
    ].join(';');
    pc.appendChild(p);
  }}

  /* ── stationary sparkles — pulse in place ── */
  var sparkCols=[
    'rgba(255,220,60,1)',
    'rgba(255,245,160,1)',
    'rgba(232,184,75,1)'
  ...
  for(var j=0;j<12;j++){{
    ...
    s.style.cssText=[
      ...
      'background:'+scol,
      'border-radius:50%',
      'box-shadow:0 0 '+sglow+'px '+sglow+'px rgba(255,200,50,0.7)',
      'animation:particleSparkle '+(Math.random()*3+2).toFixed(1)+'s ease-in-out '+(Math.random()*6).toFixed(1)+'s infinite',
      'pointer-events:none'
    ].join(';');
    pc.appendChild(s);
  }}
...
/* ── word glitter — particles fall off the shimmer text ── */
(function(){{
  var wg = document.getElementById('wordglitter');
  var gc = [
    'rgba(255,215,50,1)',
    'rgba(255,235,120,0.97)',
    'rgba(232,184,75,0.95)',
    'rgba(255,255,160,0.92)',
    'rgba(248,210,60,0.98)'
  ];
  ...
  function spawnFaller() {{
    ...
    /* originate from within the quote text band — ~32%–58% down the screen */
    var sx = (0.10 + Math.random() * 0.80) * W;
    var sy = (0.32 + Math.random() * 0.26) * H;

    /* fall path — gravity down, slight horizontal wander, spin */
    ...
    /* shape — mix of dots and slight diamonds */
    ...
    p.style.cssText = [
      ...
      'box-shadow:0 0 ' + gl + 'px ' + gl + 'px rgba(255,200,50,0.65)',
      'pointer-events:none'
    ].join(';');
  ...
  /* burst when text appears (~1.4s in), then continuous trickle */
